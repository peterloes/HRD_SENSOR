 ...
 * directly from the sensor controller.
 *
 * @param[in] pItem
 ...
 * 	or NULL if there was an error, e.g. a read error from the sensor
 * 	controller.
 *
 * @warning
...
SBS_CMD		 regAddr;	// command, i.e. the register address to read
	...
	if (data != 0)
	{
	    ...
	    if (SensorRegReadBlock (regAddr, dataBuf, sizeof(dataBuf)) < 0)
	...
	else
	{
	    ...
	    data = SensorRegReadWord (regAddr);
	    if (data < 0)
		return NULL;	// READ ERROR
	}
    }
